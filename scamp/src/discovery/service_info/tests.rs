...
#[test]
fn test_parse() {
    let info = AnnouncementBody::parse(include_str!("../../../samples/service_info_packet_v3_data.json")).unwrap();
...
fn test_unrle_plain_values() {
    let obj: serde_json::Map<String, serde_json::Value> = serde_json::from_value(json!({"v": ["a", "b", "c"]})).unwrap();
...
fn test_unrle_with_repeats() {
    let obj: serde_json::Map<String, serde_json::Value> = serde_json::from_value(json!({"v": [[3, "x"], "y"]})).unwrap();
...
fn test_unrle_missing_optional() {
    let obj: serde_json::Map<String, serde_json::Value> = serde_json::from_value(json!({})).unwrap();
...
fn test_unrle_single_value_broadcast() {
    let obj: serde_json::Map<String, serde_json::Value> = serde_json::from_value(json!({"v": "same"})).unwrap();
    let result: Vec<String> = parse::unrle(&obj, "v", true, 3).unwrap();
...
fn test_unrle_bad_repeat_chunk() {
    let obj: serde_json::Map<String, serde_json::Value> = serde_json::from_value(json!({"v": [[1, 2, 3]]})).unwrap();

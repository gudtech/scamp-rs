...
// ---- Perl wire fixture tests ----
// These parse real packets captured from the canonical Perl implementation.
...
fn test_perl_reply_ok() {
    ...
    match Packet::parse(&buf) {
        ParseResult::Success { packet, .. } => {
            ...
            assert_eq!(hdr.request_id.0, 1);
            assert!(hdr.error.is_none()); // null → None

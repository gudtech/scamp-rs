...
use std::fmt;
use std::mem;
#[cfg(feature = "runt∑ime")] use std::net::SocketAddr;
use std::sync::Arc;
...
///
/// This structure is used to configure options for an HTTP listener connection.
///
/// If you don't have need to manage connections yourself, consider using the
/// higher-level [Server](super) API.
#[derive(Clone, Debug)]
...
impl Http {
    /// Creates a new instance of the HTTP protocol, ready to spawn a listener or
...
impl<E> Http<E> {
    ...
    /// for the listener to respond. Setting this to `false` will
...
pub(crate) mod spawn_all {
    ...
    //
    // Doing this allows for the listener to conditionally require `Send` futures,
